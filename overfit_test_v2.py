#!/usr/bin/env python
"""
Overfitting test v2: Train delta-z to improve noisy initial depth.
This is more realistic - we start with a corrupted depth and train the model to fix it.
"""
...
print("=" * 80)
print("OVERFITTING TEST v2 - Train Delta-Z on Noisy Input")
print("=" * 80)
...
if loss_variance < 0.1:
    print("✓ Loss curve stable (low variance)")
else:
    print(f"⚠️  Loss curve noisy (variance: {loss_variance:.6f})")
...
if weight_stats:
    ...
    if avg_grad < 1e-8:
        print("⚠️  Very small gradients - model may not be learning")
    elif avg_grad > 1.0:
        print("⚠️  Large gradients - consider lower learning rate")
    else:
        print("✓ Gradient magnitude in healthy range")

print("\n" + "=" * 80)
print("OVERFITTING TEST v2 COMPLETE")
print("=" * 80)
